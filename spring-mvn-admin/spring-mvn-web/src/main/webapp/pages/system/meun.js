var TheMeun = function() {
	...
	var handleSubData = function(subdata){
		...
		jQuery.each(subdata, function(index, val){		
			if(!Sunline.isNull(val.haschild) && val.haschild === "Y"){
				substr+='{"a_attr":{"id":"'+val.auth_cd+'","rank":"'+val.rank+'","sortno":"'+val.sortno+'","children":'+
				handleSubData(val.children)+'},"text":"'+val.menu_name+'","icon":"'+val.iconfg+'","children":'+
				handleSubData(val.children)+'}';
			}else{
				substr+='{"a_attr":{"id":"'+val.auth_cd+'","rank":"'+val.rank+'","sortno":"'+val.sortno+'"},"text":"'+val.menu_name+'","icon":"'+val.iconfg+'"}';
